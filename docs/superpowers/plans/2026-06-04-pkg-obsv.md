...
**Goal:** Build `github.com/inovacc/mantle/pkg/obsv` — a one-call OpenTelemetry bootstrap (logs+traces+metrics providers, OTLP gRPC/HTTP + stdout exporters, resource, propagation, runtime metrics) exposing a `Stack` whose `LogSink()` plugs into `logger.WithSink`, with `Tracer()/Meter()` accessors and a unified `Shutdown`.
